...
modeButtons.forEach(modeButton=>{
  if(modeButton.classList.contains('active')){
    ...
    modeName=modeButton.dataset.name;
    modeDisplay.innerText=`for ${modeButton.dataset.name}`;
  }
  modeButton.addEventListener('click', ()=>{
    modeButtons.forEach(subModeButton=>{
  ...
  reader.onload=()=>{
    ...
    image.onload= async ()=>{
      ...
      if(vers==correctName){
        ...
        errorMessage.innerText+=`[ERROR]使用されているテンプレートが${correctName}のものではありません。または、右下の開催回番号枠内が汚れている可能性があります\n`
